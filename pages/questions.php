<?php
	...
	if (!empty($_SESSION['auth'] and $_SESSION['user']['superuser'] == 1)) {
		...
		function showQuestionsTable($pdo, $page, $count, $from, $numOfPages, $siteroot, $cssroot)
		{
			$topInfo = 'Вопросы';
			...
			foreach ($questions as $question) {
				...
				$content .= "<tr>
								...
								<td>{$question['mark']}</td>
								<td>$textStatus</td>
								<td><a href=\"editQuestion.php?id={$question['id']}\">Редактировать</a></td>
								<td><a href=\"?changeStatus={$question['id']}&status={$question['status']}\">Удалить/восстановить</a></td>
							</tr>";
			}
			$content .= '</tbody></table>';
		...
		function changeStatus($pdo, $siteroot)
		{
			if (isset($_GET['changeStatus'])) {
				...
				if($status == 1) {
					...
					$_SESSION['message'] = ['text' => 'Вопрос был отключен!', 
											'status' => 'success'];
					...
					$_SESSION['message'] = ['text' => 'Вопрос снова активен!', 
											'status' => 'success'];
		...
		showQuestionsTable($pdo, $page, $count, $from, $numOfPages, $siteroot, $cssroot);
