	...
	if (!empty($_SESSION['auth'] and $_SESSION['user']['superuser'] == 1)) {
		...
		function showRightTable($pdo, $cssroot, $siteroot)
		{
			...
			if (isset($_GET['selectRight'])) {
				...
				if (!$rightName) {
					...
					$query = "SELECT rk.id AS rightklnId, k.name AS kln 
					FROM rightskln rk 
					inner join klns k on rk.klns = k.id 
					WHERE rights = ?";
					$stmt = $pdo->prepare($query);
					...
					foreach ($klns as $kln) {
						...
						$content .= "<tr>
										<td>{$kln['kln']}</td>
										<td><a href=\"?changeStatus={$kln['rightklnId']}\">Удалить</a></td>
									</tr>";
					}
					$content .= '</tbody></table>';
				}	
		...
		function changeStatus($pdo, $siteroot)
		{
			if (isset($_GET['changeStatus'])) {
				...
				$query = "DELETE FROM rightskln WHERE `id` = :id";
				$params = [
					':id' => $id
				];
				$stmt = $pdo->prepare($query);
				...
				$_SESSION['message'] = ['text' => 'Право КЛН было удалено', 
				'status' => 'success'];
			} 
		}
